...
class FrameCache:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        Args:
            cache_dir: Directory to store cache files. If None, uses ~/.cover_selector_cache
        """
    ...
    def _frame_hash(self, frame_bytes: bytes) -> str:
        """Compute content hash of frame (MD5)."""
        return hashlib.md5(frame_bytes).hexdigest()

    def _get_cache_path(self, frame_hash: str, config_hash: str) -> Path:
        """Get cache file path for frame + config combination."""
        # Format: cache_dir/frame_hash_config_hash.json
    ...
    def get(self, frame_bytes: bytes, config_hash: str) -> Optional[Dict]:
        ...
        Args:
            frame_bytes: Raw frame bytes (for content hashing)
            config_hash: Hash of current config (for invalidation)

        Returns:
            Cached features dict if found, None otherwise
        """
        ...
        try:
            with open(cache_path, "r") as f:
                cached = json.load(f)
            self.stats["hits"] += 1
            logger.debug(f"Cache hit for frame {frame_hash[:8]}")
            return cached
        except (json.JSONDecodeError, IOError) as e:
            logger.warning(f"Cache corruption detected: {e}. Recomputing.")
            self.stats["errors"] += 1
    ...
    def put(self, frame_bytes: bytes, config_hash: str, features: Dict) -> bool:
        ...
        try:
            ...
            self.stats["writes"] += 1
            logger.debug(f"Cached features for frame {frame_hash[:8]}")
            return True
        except (TypeError, IOError) as e:
            logger.warning(f"Failed to cache frame features: {e}")
            self.stats["errors"] += 1
            return False

    def clear(self) -> int:
        """Clear all cache entries. Returns count of cleared files."""
        try:
            count = 0
