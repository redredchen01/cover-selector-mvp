...
class FrameFeatures(BaseModel):
    ...
    laplacian_variance: float = Field(0.0, ge=0, description="Laplacian variance for sharpness")
    edge_density: float = Field(0.0, ge=0, le=1, description="Edge pixel ratio (0-1)")
    ...
    overexposure_score: float = Field(0.0, ge=0, le=100, description="Overexposure percentage")
    underexposure_score: float = Field(0.0, ge=0, le=100, description="Underexposure percentage")

    # OCR features
    ocr_text_count: int = Field(0, ge=0, description="Number of text regions detected")
    ocr_text_area_ratio: float = Field(0.0, ge=0, le=1, description="Ratio of text area to image")
    bottom_subtitle_ratio: float = Field(0.0, ge=0, le=1, description="Bottom subtitle area ratio")
    corner_text_ratio: float = Field(0.0, ge=0, le=1, description="Corner text area ratio")
    center_text_ratio: float = Field(0.0, ge=0, le=1, description="Center text area ratio")

    # Face detection features
    face_count: int = Field(0, ge=0, description="Number of faces detected")
    largest_face_ratio: float = Field(0.0, ge=0, le=1, description="Largest face ratio to image")
    face_edge_cutoff_ratio: float = Field(0.0, ge=0, le=1, description="Face cutoff at image edges")
    ...
    subject_center_offset: float = Field(0.0, ge=0, le=1, description="Subject offset from center")
